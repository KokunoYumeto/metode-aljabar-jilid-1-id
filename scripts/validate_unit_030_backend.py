#!/usr/bin/env python3
"""Regenerate and fail-closed validate the canonical Unit 030 backend."""
...
import generate_unit_030_backend as gen


ROOT = Path(__file__).resolve().parents[1]
GENERATOR = ROOT / "scripts/generate_unit_030_backend.py"
SHARED_VALIDATOR = ROOT / "scripts/validate_backend.py"
SCHEMA = ROOT / "backend/schema/open-math-corpus-unit.schema.v1.json"
DATA = ROOT / "backend/data/unit-030-bab-4-deret-komposisi-grup.json"
EVIDENCE = ROOT / "qa/unit-030-evidence/backend-validation.json"
...
EXPECTED_PREFIX_COUNTS = {
    "concept/": 13,
    "surface/unit-030/environment/": 26,
    "surface/unit-030/label/": 10,
    "surface/unit-030/reference/ordinary/": 8,
    "surface/unit-030/reference/equation/": 1,
    "surface/unit-030/item/": 0,
    "surface/unit-030/diagram-arrow/": 23,
    "surface/unit-030/protected-math-zone/": 88,
    "surface/unit-030/terminology-row/": 8,
    "correction/o013-li-u030-cor-": 1,
    "protected-text-localization/o013-li-u030-loc-": 2,
    "provenance/o013-li-u030-production": 1,
}
EXPECTED_RIGHTS = {
...
EXPECTED_QA = {
    ...
    "qa/unit-030/source-review",
    "qa/unit-030/candidate-artifact",
    "qa/unit-030/candidate-check",
    "qa/unit-030/canonical-integration",
    "qa/unit-030/source-corrections",
    "qa/unit-030/protected-text-localizations",
    "qa/unit-030/terminology-control",
    "qa/unit-030/terminology-delta",
    "qa/unit-030/terminology-evidence",
    "qa/unit-030/prepromotion-evidence",
    "qa/unit-030/citation-closure",
    "qa/unit-030/build-log",
    "qa/unit-030/visual-preflight",
    "qa/unit-030/structure-and-pdf-qa",
    "qa/unit-030/render-hash-inventory",
    "qa/unit-030/all-page-visual-review",
}
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("Unit 030 backend validation refused: " + message)
...
def run(command: list[str]) -> str:
    ...
    if completed.returncode:
        raise SystemExit(
            "Unit 030 backend validation refused:\n"
            + completed.stdout
            + completed.stderr
        )
...
def main() -> None:
    ...
    for marker in ('"concepts": 181', '"diagrams": 6', '"protected_math_zones": 88', '"citations": 0'):
    ...
    require(b"unit-026" not in all_generated, "stale Unit 026 identifier leaked into Unit 030 outputs")
    require(b"u026" not in all_generated.lower(), "stale U026 identifier leaked into Unit 030 outputs")
    require(b"unit-029" not in all_generated, "stale Unit 029 identifier leaked into Unit 030 outputs")
    require(b"u029" not in all_generated.lower(), "stale U029 identifier leaked into Unit 030 outputs")
    ...
    require(
        ...
        and data["dataset_id"] == "urn:uuid:" + str(uuid.uuid5(namespace, "dataset/unit-030/id-id")),
    ...
    unit = data["unit"]
    require(unit["stable_key"] == "unit/bab-4-deret-komposisi-grup", "unit key drift")
    require(unit["order"] == 30, "unit order drift")
    require(
        unit["source_local_id"] == "chapter4.tex:796-935; substantive authority map 796-934 to target 794-932",
        "authority description drift",
    )
    require(
        unit["source_binding"]["line_start"] == 796
        and unit["source_binding"]["line_end"] == 935
    ...
    require(
        unit["target_binding"]["line_start"] == 794
        and unit["target_binding"]["line_end"] == 932
    ...
    section = sections[0]
    require(
        section["stable_key"] == "unit/bab-4-deret-komposisi-grup/section/01"
        and section["order"] == 1
        and section["source_binding"]["line_start"] == 796
        and section["source_binding"]["line_end"] == 935
        and section["target_binding"]["line_start"] == 794
        and section["target_binding"]["line_end"] == 932,
        "section boundary or order drift",
    )

    concepts = data["concepts"]
    concept_keys = tuple(item["stable_key"] for item in concepts)
    require(len(concepts) == 181, f"expected 181 concept-compatible entities, got {len(concepts)}")
    ...
    for ordinal, (source_term, target_term) in enumerate(gen.TERMINOLOGY_PAIRS, 1):
        labels = concept_by_key[f"surface/unit-030/terminology-row/{ordinal:03d}"]["labels"]
    ...
    provenance_text = " ".join(item["text"] for item in concept_by_key["provenance/o013-li-u030-production"]["labels"])
    ...
    require(len(citations) == 0 and unit["citation_ids"] == [], "unexpected native citation records")
    diagrams = data["diagrams"]
    require(len(diagrams) == 6 and unit["diagram_ids"] == [item["id"] for item in diagrams], "expected six native diagram records")
    ...
    require(
        [item["ordinal_in_unit"] for item in indexes] == [1, 2, 3, 4, 5, 6]
    ...
    admission_scope = qa["qa/unit-030/admission-gate"]["scope"]
    for token in (
        MODEL,
        "796-935",
        "139-record",
        "26 active environment pairs",
        "ten labels",
        "nine references",
        "six index entries",
        "23 diagram arrows",
        "88 protected mathematical zones",
        "eight admitted terminology rows",
        "one source correction",
        "two protected-text localizations",
        "six tikzcd diagrams",
    ...
    witness_by_qa = {
        "qa/unit-030/admission-gate": gen.VISUAL_REVIEW,
        "qa/unit-030/source-review": gen.REVIEW,
        "qa/unit-030/candidate-artifact": gen.CANDIDATE,
        "qa/unit-030/candidate-check": gen.CANDIDATE_GATE,
        "qa/unit-030/canonical-integration": gen.STRUCTURE_GATE,
        "qa/unit-030/source-corrections": gen.REVIEW,
        "qa/unit-030/protected-text-localizations": gen.REVIEW,
        "qa/unit-030/terminology-control": gen.TERMINOLOGY,
        "qa/unit-030/terminology-delta": gen.TERMINOLOGY_DELTA,
        "qa/unit-030/terminology-evidence": gen.TERMINOLOGY_AUDIT,
        "qa/unit-030/prepromotion-evidence": gen.PREPROMOTION_AUDIT,
        "qa/unit-030/citation-closure": gen.BIBLIOGRAPHY,
        "qa/unit-030/build-log": gen.FINAL_LOG,
        "qa/unit-030/visual-preflight": gen.VISUAL_PREFLIGHT,
        "qa/unit-030/structure-and-pdf-qa": gen.STRUCTURE_PDF_QA,
        "qa/unit-030/render-hash-inventory": gen.RENDER_HASH_INVENTORY,
        "qa/unit-030/all-page-visual-review": gen.VISUAL_REVIEW,
    }
    ...
    report = {
        "status": "PASS",
        "unit": "unit-030-bab-4-deret-komposisi-grup",
        "authority": "chapter4.tex:796-935 (blank line 935 omitted from mapping)",
        "target": "chapter4.tex:794-932",
        ...
        "counts": {
            "sections": 1,
            "concepts": 181,
            "textual_environment_markers": 52,
            "textual_environment_pairs": 26,
            "active_environment_pairs": 26,
            "labels": 10,
            "ordinary_references": 8,
        ...
        "checks": {
            ...
            "no_stale_unit_026_or_029_ids_or_data": "PASS",
            ...
            "two_protected_text_localizations": "PASS",
