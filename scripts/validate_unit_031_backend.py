#!/usr/bin/env python3
"""Regenerate and fail-closed validate the canonical Unit 031 backend."""
...
import generate_unit_031_backend as gen


ROOT = Path(__file__).resolve().parents[1]
GENERATOR = ROOT / "scripts/generate_unit_031_backend.py"
SHARED_VALIDATOR = ROOT / "scripts/validate_backend.py"
SCHEMA = ROOT / "backend/schema/open-math-corpus-unit.schema.v1.json"
DATA = ROOT / "backend/data/unit-031-bab-4-grup-solvabel-dan-nilpoten.json"
EVIDENCE = ROOT / "qa/unit-031-evidence/backend-validation.json"
...
EXPECTED_PREFIX_COUNTS = {
    "concept/": 19,
    "surface/unit-031/environment/": 31,
    "surface/unit-031/label/": 6,
    "surface/unit-031/reference/ordinary/": 7,
    "surface/unit-031/reference/equation/": 0,
    "surface/unit-031/item/": 14,
    "surface/unit-031/diagram-arrow/": 3,
    "surface/unit-031/drawing-command/": 2,
    "surface/unit-031/protected-math-zone/": 326,
    "surface/unit-031/terminology-row/": 14,
    "correction/o013-li-u031-cor-": 1,
    "digital-reflow/o013-li-u031-reflow-": 1,
    "protected-text-localization/o013-li-u031-loc-": 8,
    "provenance/o013-li-u031-production": 1,
}
EXPECTED_RIGHTS = {
...
EXPECTED_QA = {
    ...
    "qa/unit-031/source-review",
    "qa/unit-031/candidate-artifact",
    "qa/unit-031/candidate-check",
    "qa/unit-031/canonical-integration",
    "qa/unit-031/source-corrections",
    "qa/unit-031/digital-reflow",
    "qa/unit-031/protected-text-localizations",
    "qa/unit-031/terminology-control",
    "qa/unit-031/terminology-delta",
    "qa/unit-031/terminology-evidence",
    "qa/unit-031/prepromotion-evidence",
    "qa/unit-031/citation-closure",
    "qa/unit-031/build-log",
    "qa/unit-031/visual-preflight",
    "qa/unit-031/structure-and-pdf-qa",
    "qa/unit-031/render-hash-inventory",
    "qa/unit-031/all-page-visual-review",
}
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("Unit 031 backend validation refused: " + message)
...
def run(command: list[str]) -> str:
    ...
    if completed.returncode:
        raise SystemExit(
            "Unit 031 backend validation refused:\n"
            + completed.stdout
            + completed.stderr
        )
...
def main() -> None:
    ...
    for marker in ('"concepts": 433', '"diagrams": 3', '"protected_math_zones": 326', '"citations": 1', '"digital_reflows": 1'):
    ...
    require(b"unit-026" not in all_generated, "stale Unit 026 identifier leaked into Unit 031 outputs")
    require(b"u026" not in all_generated.lower(), "stale U026 identifier leaked into Unit 031 outputs")
    require(b"unit-029" not in all_generated, "stale Unit 029 identifier leaked into Unit 031 outputs")
    require(b"u029" not in all_generated.lower(), "stale U029 identifier leaked into Unit 031 outputs")
    require(b"unit-030" not in all_generated, "stale Unit 030 identifier leaked into Unit 031 outputs")
    require(b"u030" not in all_generated.lower(), "stale U030 identifier leaked into Unit 031 outputs")
    ...
    require(
        ...
        and data["dataset_id"] == "urn:uuid:" + str(uuid.uuid5(namespace, "dataset/unit-031/id-id")),
    ...
    unit = data["unit"]
    require(unit["stable_key"] == "unit/bab-4-grup-solvabel-dan-nilpoten", "unit key drift")
    require(unit["order"] == 31, "unit order drift")
    require(
        unit["source_local_id"] == "chapter4.tex:936-1107; substantive authority map 936-1106 to target 933-1103",
        "authority description drift",
    )
    require(
        unit["source_binding"]["line_start"] == 936
        and unit["source_binding"]["line_end"] == 1107
    ...
    require(
        unit["target_binding"]["line_start"] == 933
        and unit["target_binding"]["line_end"] == 1103
    ...
    section = sections[0]
    require(
        section["stable_key"] == "unit/bab-4-grup-solvabel-dan-nilpoten/section/01"
        and section["order"] == 1
        and section["source_binding"]["line_start"] == 936
        and section["source_binding"]["line_end"] == 1107
        and section["target_binding"]["line_start"] == 933
        and section["target_binding"]["line_end"] == 1103,
        "section boundary or order drift",
    )

    concepts = data["concepts"]
    concept_keys = tuple(item["stable_key"] for item in concepts)
    require(len(concepts) == 433, f"expected 433 concept-compatible entities, got {len(concepts)}")
    ...
    for ordinal, (source_term, target_term) in enumerate(gen.TERMINOLOGY_PAIRS, 1):
        labels = concept_by_key[f"surface/unit-031/terminology-row/{ordinal:03d}"]["labels"]
    ...
    for reflow_id, source_lines, target_lines, source_layout, target_layout in gen.DIGITAL_REFLOWS:
        key = f"digital-reflow/{reflow_id.casefold()}"
        text = " ".join(item["text"] for item in concept_by_key[key]["labels"])
        for token in (
            reflow_id,
            gen.PREPROMOTION_AUDIT,
            source_layout,
            target_layout,
            *(str(line) for line in source_lines),
            *(str(line) for line in target_lines),
        ):
            require(token in text, f"{reflow_id} provenance lacks {token!r}")
    ...
    provenance_text = " ".join(item["text"] for item in concept_by_key["provenance/o013-li-u031-production"]["labels"])
    ...
    composition_prerequisite = next(
        item for item in data["prerequisites"]
        if item["stable_key"] == "prerequisite/composition-series-and-jordan-holder-theory"
    )
    require(
        composition_prerequisite["source_evidence"] == {"path": gen.SOURCE, "line_start": 796, "line_end": 935},
        "Unit 030 prerequisite evidence drift",
    )

    citations = data["citations"]
    require(
        len(citations) == 1
        and unit["citation_ids"] == [citations[0]["id"]]
        and citations[0]["stable_key"] == "citation/unit-031/01-ft63"
        and citations[0]["bib_key"] == "FT63"
        and citations[0]["bibliography_path"] == gen.BIBLIOGRAPHY
        and citations[0]["bibliography_sha256"] == gen.BIBLIOGRAPHY_ID[1]
        and citations[0]["source_line"] == 1033
        and citations[0]["target_line"] == 1030,
        "FT63 native citation record drift",
    )
    diagrams = data["diagrams"]
    require(len(diagrams) == 3 and unit["diagram_ids"] == [item["id"] for item in diagrams], "expected three native diagram records")
    ...
    require(
        ...
        == Counter({"main": 6, "sym1": 3}),
        "main/symbol index census drift",
    )
    require(
    ...
    admission_scope = qa["qa/unit-031/admission-gate"]["scope"]
    for token in (
        MODEL,
        "936-1107",
        "171-record",
        "31 active environment pairs",
        "six labels",
        "seven references",
        "one FT63 citation",
        "nine index entries",
        "three diagram arrows",
        "326 protected mathematical zones",
        "fourteen admitted terminology rows",
        "one proof repair",
        "one target-only digital reflow",
        "eight protected-text localizations",
        "two tikzpicture diagrams and one tikzcd diagram",
    ...
    witness_by_qa = {
        "qa/unit-031/admission-gate": gen.VISUAL_REVIEW,
        "qa/unit-031/source-review": gen.REVIEW,
        "qa/unit-031/candidate-artifact": gen.CANDIDATE,
        "qa/unit-031/candidate-check": gen.CANDIDATE_GATE,
        "qa/unit-031/canonical-integration": gen.STRUCTURE_GATE,
        "qa/unit-031/source-corrections": gen.REVIEW,
        "qa/unit-031/digital-reflow": gen.PREPROMOTION_AUDIT,
        "qa/unit-031/protected-text-localizations": gen.REVIEW,
        "qa/unit-031/terminology-control": gen.TERMINOLOGY,
        "qa/unit-031/terminology-delta": gen.TERMINOLOGY_DELTA,
        "qa/unit-031/terminology-evidence": gen.TERMINOLOGY_AUDIT,
        "qa/unit-031/prepromotion-evidence": gen.PREPROMOTION_AUDIT,
        "qa/unit-031/citation-closure": gen.BIBLIOGRAPHY,
        "qa/unit-031/build-log": gen.FINAL_LOG,
        "qa/unit-031/visual-preflight": gen.VISUAL_PREFLIGHT,
        "qa/unit-031/structure-and-pdf-qa": gen.STRUCTURE_PDF_QA,
        "qa/unit-031/render-hash-inventory": gen.RENDER_HASH_INVENTORY,
        "qa/unit-031/all-page-visual-review": gen.VISUAL_REVIEW,
    }
    ...
    report = {
        "status": "PASS",
        "unit": "unit-031-bab-4-grup-solvabel-dan-nilpoten",
        "authority": "chapter4.tex:936-1107 (blank line 1107 omitted from mapping)",
        "target": "chapter4.tex:933-1103",
        ...
        "counts": {
            "sections": 1,
            "concepts": 433,
            "textual_environment_markers": 62,
            "textual_environment_pairs": 31,
        ...
        "checks": {
            ...
            "no_stale_unit_026_029_or_030_ids_or_data": "PASS",
            ...
            "one_declared_proof_repair": "PASS",
            "one_target_only_digital_reflow": "PASS",
            "eight_protected_text_localizations": "PASS",
