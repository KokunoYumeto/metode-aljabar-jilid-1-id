[CmdletBinding()]
param(
    [string]$OutputDirectory = "build/unit-026-replay"
)
...
$expectedTargetLineRecords = 1899
$expectedTargetBytes = 163745
$expectedTargetSha256 = "fc3fd6ef470d41f146456bfc889eb7c7ec84bb48890f1b23f18e51a195e7d463"
$spanStart = 179
$spanEnd = 365
$expectedSpanLineRecords = 187
$expectedSpanBytes = 19424
$expectedSpanSha256 = "a3745af3387afbee36e1c39a91ab531efc0f97d10b1fb6bc95d4505143c9de87"
...
if ($targetText.Contains("`r")) {
    throw "Unit 026 canonical target must use LF-only line endings"
}
...
if ($targetLineRecords -ne $expectedTargetLineRecords -or $targetItem.Length -ne $expectedTargetBytes -or $targetHash -ne $expectedTargetSha256) {
    throw "Unit 026 canonical target identity mismatch: records=$targetLineRecords bytes=$($targetItem.Length) sha256=$targetHash"
}
...
if ($spanLines.Count -ne $expectedSpanLineRecords -or $spanBytes.Length -ne $expectedSpanBytes -or $spanHash -ne $expectedSpanSha256) {
    throw "Unit 026 canonical span identity mismatch: records=$($spanLines.Count) bytes=$($spanBytes.Length) sha256=$spanHash"
}
...
$jobName = "unit-026-bab-4-homomorfisme-dan-grup-hasil-bagi"
