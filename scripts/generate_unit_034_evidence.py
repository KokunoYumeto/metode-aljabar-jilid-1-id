#!/usr/bin/env python3
"""Finalize fail-closed PDF and all-page visual evidence for Unit 034.

Run only after the Unit 034 preflight passes, every full-resolution page has
been inspected in both renderer outputs, and build J has been copied byte-for-
byte to the release artifact. The findings JSON must contain one substantive
finding for every page and no actionable defect.
"""
...
import preflight_unit_034 as preflight


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PREFLIGHT = ROOT / "build/unit-034-preflight-final-i-j/preflight-observation.json"
DEFAULT_FINDINGS = ROOT / "build/unit-034-visual-findings.json"
DEFAULT_ARTIFACT = ROOT / "artifacts/unit-034-bab-4-limit-dan-kompletisasi-grup-id.pdf"
DEFAULT_EVIDENCE = ROOT / "qa/unit-034-evidence"
DEFAULT_REPORT = ROOT / "qa/UNIT_034_VISUAL_QA_20260827.md"
DEFAULT_PREFLIGHT_REPORT = ROOT / "qa/UNIT_034_VISUAL_PREFLIGHT_20260827.md"
DEFAULT_FINAL_LOG = ROOT / "qa/UNIT_034_BUILD_FINAL.log"
...
def bind_observation(path: Path) -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    assert packet["schema_version"] == 1 and packet["unit"] == "O013-LI-U034"
    ...
    expected_underfull = packet["configuration"]["expected_underfull_hboxes"]
    assert isinstance(expected_underfull, int) and expected_underfull >= 0
    ...
    for name in ("build-i", "build-j"):
        ...
        assert packet["logs"][name]["nonfatal_underfull_hboxes"] == expected_underfull
        builds[name] = pdf
...
def sanitize_log(raw_path: Path) -> tuple[str, dict[str, int]]:
    ...
    return sanitized, {
        ...
        "line_records_preserved": len(raw.splitlines()),
...
def write_reports(
    ...
    underfull = log["nonfatal_underfull_hboxes"]
    report.write_text(
        f"""# Unit 034 visual and PDF QA - 2026-08-27
...
- Final diagnostics are zero for errors, unresolved references/citations, missing characters, empty targets, and overfull boxes. Exactly {underfull} nonfatal underfull hboxes remain and were visually inspected.
...
""",
    ...
    preflight_report.write_text(
        f"""# Unit 034 visual preflight - 2026-08-27
...
{underfull} visually non-actionable underfull hboxes and fixed toolchain
advisories remain disclosed in the evidence packet.
...
def main() -> None:
    ...
    expected_underfull = observation["configuration"]["expected_underfull_hboxes"]
    ...
    log = preflight.inspect_log(DEFAULT_FINAL_LOG, pages, expected_underfull)
    ...
    contact_sheets = []
    with tempfile.TemporaryDirectory(prefix="unit034-evidence-") as temporary:
        work = Path(temporary)
        for engine in renderings:
