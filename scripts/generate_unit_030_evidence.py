#!/usr/bin/env python3
"""Generate deterministic PDF and render evidence for O013 Unit 030."""
...
OUT = ROOT / "qa/unit-030-evidence"
REPORT = ROOT / "qa/UNIT_030_VISUAL_QA_20260826.md"
PREFLIGHT = ROOT / "qa/UNIT_030_VISUAL_PREFLIGHT_20260826.md"
DPI = 144
PAGES = 7
PIXEL_SIZE = (998, 1418)
EDGE_BAND = 3
PDF_NAME = "unit-030-bab-4-deret-komposisi-grup.pdf"
DOCS = {
    "build-i": (
        ROOT / "build/unit-030-clean-i" / PDF_NAME,
        91_960,
        "cb5b2c5279e7c5574457846873a456cb644d2a1d54ace7ddc975378e036e00cd",
    ),
    "build-j": (
        ROOT / "build/unit-030-clean-j" / PDF_NAME,
        91_961,
        "43ad2ffa2516f2f4394bcb82ad2e585f21c1e9e36a87870f4406a78597f18d74",
    ),
    "artifact": (
        ROOT / "artifacts/unit-030-bab-4-deret-komposisi-grup-id.pdf",
        91_961,
        "43ad2ffa2516f2f4394bcb82ad2e585f21c1e9e36a87870f4406a78597f18d74",
    ),
}
FINAL_LOG = (
    ROOT / "qa/UNIT_030_BUILD_FINAL.log",
    76_370,
    "728d7f4f2845e87132e6bf494784a16eced1b9f9f2644f97a10ae477a9df43f9",
)
RAW_FINAL_LOG = ROOT / "build/unit-030-clean-j" / "unit-030-bab-4-deret-komposisi-grup.log"
EXPECTED_METADATA = {
    "/Creator": "LaTeX with hyperref",
    "/Title": "Metode Aljabar, Jilid 1: Arsitektur Dasar - Unit 30: Deret Komposisi Grup",
    "/Subject": "Terjemahan Bahasa Indonesia independen; Bagian 4.6 lengkap",
    "/Author": "Wen-Wei Li",
    "/Keywords": "aljabar, teori grup, deret normal, deret komposisi, Lema Zassenhaus, Jordan–Hölder, id-ID",
    "/Producer": "MiKTeX-dvipdfmx (20260404)",
    "/CreationDate": "D:20260825000000Z",
}
EXPECTED_OUTLINE = [
    ("4.6 Deret Komposisi Grup", 3),
    ("Daftar Pustaka", 7),
    ("Indeks Istilah", 7),
]
VISUAL_FINDINGS = [
    "Hierarki sampul, judul Unit 30, panel ruang lingkup, tanggal, dan margin seimbang serta mudah dibaca.",
    "Atribusi edisi dan sumber, ISBN, lisensi, non-endorsement, provenance model, tautan, dan lencana CC tampil utuh tanpa tabrakan.",
    "Judul Bagian 4.6 serta definisi deret normal, sentral, dan komposisi tersusun jelas dengan lebar teks terpusat.",
    "Lema Zassenhaus dan keenam diagram tikzcd terbaca jelas; label subgrup dan subgrup normal terlokalisasi tanpa merusak topologi.",
    "Teorema Penghalusan Schreier, relasi Jordan--Hölder, dan rumus-rumus terkait mengalir alami; kotak teorema dan rujukan tidak terpotong.",
    "Bukti faktor komposisi grup abelian, daftar pustaka kosong yang dinyatakan, dan indeks istilah run-in mengisi halaman akhir secara alami dengan margin aman.",
    "Halaman indeks dan penutup tetap memiliki margin bawah aman; tidak ada ruang kosong yang mengganggu atau elemen yang terpotong.",
]
...
def sanitized_build_log(raw_text: str) -> tuple[str, dict[str, int]]:
    ...
    absolute_build_prefix = str((ROOT / "build/unit-030-clean-j").resolve())
    relative_build_prefix = r"build\unit-030-clean-j"
    ...
    # The MiKTeX terminal wraps long lane paths at arbitrary columns.  Replace
    # both the contiguous and wrapped spellings while preserving every LF.
    ...
    # Terminal wrapping can leave a leading separator before the relative
    # build path; the safety gates below, not this cosmetic count, decide
    # whether any profile-bearing path survived.
...
def render_gate(renderings: dict[str, Any]) -> dict[str, Any]:
    ...
    for engine in ("poppler", "mupdf"):
        ...
        for left, right, key in (("build-i", "build-j", "build-i_vs_build-j"), ("build-j", "artifact", "build-j_vs_artifact")):
            ...
            comparisons[engine][key] = {"all_7_decoded_pixel_identical": True, "pages": rows}
...
def inspect_document(name: str, path: Path, work: Path) -> dict[str, Any]:
    ...
    assert len(destinations) == 25 and all(1 <= page <= PAGES for page in destinations.values())
    ...
    assert action_counts == Counter({"/GoTo": 11, "/URI": 3})
    ...
    assert len(font_records) == 21 and all(item["embedded"] for item in font_records)
    assert len(font_rows) == 21 and all(re.search(r"\syes\s+(?:yes|no)\s+(?:yes|no)\s+\d+\s+\d+\s*$", row) for row in font_rows)
    ...
    assert nul_counts == {"pypdf": 0, "pdftotext_layout": 0, "mupdf": 0}
    return {
...
def final_log_record() -> dict[str, Any]:
    ...
    assert (record["latex_release_warnings"], record["xecjk_warnings"], record["braids_warnings"], record["fontspec_CJK_advisories"], len(record["underfull_hbox_badness"]), len(record["underfull_vbox_badness"])) == (3, 1, 1, 5, 1, 4)
...
def write_reports(identities: dict[str, Any], comparisons: dict[str, Any], documents: dict[str, Any], log: dict[str, Any]) -> None:
    ...
    comparison_rows = "\n".join(f"- {engine} {pair.replace('_', ' ')}: ketujuh halaman RGB terdekode identik." for engine in comparisons for pair in comparisons[engine])
    ...
    report = f"""# Unit 030 visual and PDF QA — 2026-08-25
...
3. The final log has 3 LaTeX release warnings, 1 xeCJK warning, 1 frozen `braids` warning, 5 fontspec CJK advisories, one visually benign underfull hbox (badness {log['underfull_hbox_badness'][0]}), and four visually benign underfull vboxes (badness values {log['underfull_vbox_badness']}). Fatal/error, unresolved-reference/citation, missing-character, and overfull diagnostics are zero.
...
"""
    ...
    preflight = f"""# Unit 030 visual preflight — 2026-08-26
...
- Build I: 7 pages / {identities['build-i']['bytes']:,} bytes / SHA-256 `{identities['build-i']['sha256']}`.
- Build J: 7 pages / {identities['build-j']['bytes']:,} bytes / SHA-256 `{identities['build-j']['sha256']}`.
- Canonical artifact: 7 pages / {identities['artifact']['bytes']:,} bytes / SHA-256 `{identities['artifact']['sha256']}`; byte-identical to build J.
...
def main() -> None:
    ...
    assert resolved.parent == (ROOT / "qa").resolve() and resolved.name == "unit-030-evidence"
    if OUT.exists():
        shutil.rmtree(OUT)
    OUT.mkdir(parents=True)
    renderings: dict[str, Any] = {"poppler": {}, "mupdf": {}}
    contact_sheets = []
    with tempfile.TemporaryDirectory(prefix="unit030-evidence-") as temporary:
        work = Path(temporary)
    ...
    render_inventory = {
        ...
        "decoded_pixel_comparisons": comparisons, "edge_gate": {"outer_band_pixels": EDGE_BAND, "all_42_zero_ink": True},
