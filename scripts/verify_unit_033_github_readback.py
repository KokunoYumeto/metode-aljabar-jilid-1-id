#!/usr/bin/env python3
"""Anonymously verify every path changed by a Unit 033 publication commit."""
...
def fetch(url: str) -> tuple[bytes, str, int]:
    ...
    raise RuntimeError(f"anonymous readback failed after three attempts: {url}: {last_error}")
...
def main() -> int:
    ...
    if output.parent != ROOT / "qa" or output.exists():
        raise RuntimeError("output must be a new direct child of qa")
    ...
    if parent != args.expected_parent:
        raise RuntimeError(f"unexpected commit parent: {parent}")
    ...
    local_blobs = [(path, git_bytes("cat-file", "blob", f"{commit}:{path}")) for path in paths]
    local_total = sum(len(blob) for _, blob in local_blobs)
    if local_total != args.expected_bytes:
        raise RuntimeError(f"unexpected local blob total: {local_total} != {args.expected_bytes}")

    remote_before, endpoint = read_remote_main()
    ...
    if total != args.expected_bytes:
        raise RuntimeError(f"unexpected public-byte total: {total} != {args.expected_bytes}")
    remote_after, endpoint_after = read_remote_main()
    if remote_after != commit or endpoint_after != endpoint:
    ...
    result = {
        ...
        "parent": parent,
        "tree": tree,
        "remote_main_ref_endpoint": endpoint,
