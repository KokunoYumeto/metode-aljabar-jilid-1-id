#!/usr/bin/env python3
"""Provisional two-build PDF/render preflight for O013 Li Unit 032.

This script performs no PDF authoring and never writes under ``artifacts`` or
``qa``.  It observes two already-created clean builds, renders every page with
Poppler and MuPDF, and writes a non-admission packet under ``build`` for the
final evidence generator.
"""
...
PDF_NAME = "unit-032-bab-4-grup-bebas.pdf"
DEFAULT_BUILDS = {
    "build-i": ROOT / "build/unit-032-final-k" / PDF_NAME,
    "build-j": ROOT / "build/unit-032-final-l" / PDF_NAME,
}
EXPECTED_METADATA = {
    "/Creator": "LaTeX with hyperref",
    "/Title": "Metode Aljabar, Jilid 1: Arsitektur Dasar - Unit 32: Grup Bebas",
    "/Subject": "Terjemahan Bahasa Indonesia independen; Bagian 4.8 lengkap",
    "/Author": "Wen-Wei Li",
    "/Keywords": "aljabar, teori grup, monoid bebas, grup bebas, produk bebas, presentasi grup, Nielsen-Schreier, id-ID",
    "/Producer": "MiKTeX-dvipdfmx (20260404)",
    "/CreationDate": "D:20260826000000Z",
}
EXPECTED_OUTLINE_TITLES = ["4.8 Grup Bebas", "Daftar Pustaka", "Indeks Istilah"]
...
def require_candidate() -> dict[str, Any]:
    ...
    assert checker.returncode == 0 and checker.stdout.startswith("PASS:"), checker.stderr
    record["bounded_checker_passed"] = True
    return record


def inspect_log(path: Path, pages: int) -> dict[str, Any]:
    ...
    assert underfull_hboxes == EXPECTED_UNDERFULL_HBOXES, underfull_hboxes
    # TeX wraps long absolute output paths at the configured log-line width,
    # and can split the word "pages" itself (for example, ``pag\nes``).
    markers = re.findall(
...
def inspect_pdf(path: Path) -> dict[str, Any]:
    ...
    assert {str(k): str(v) for k, v in (reader.metadata or {}).items()} == EXPECTED_METADATA
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "build/unit-032-preflight")
    args = parser.parse_args()
    builds = {"build-i": args.build_i.resolve(), "build-j": args.build_j.resolve()}
    out = args.output.resolve()
    assert out.parent == (ROOT / "build").resolve() and out.name.startswith("unit-032-preflight")
    ...
    logs = {
        name: inspect_log(pdf.with_suffix(".log"), pages)
        for name, pdf in builds.items()
    }
    ...
    contact_sheets = []
    with tempfile.TemporaryDirectory(prefix="unit032-preflight-") as temporary:
        work = Path(temporary)
    ...
    packet = {
        ...
        "next": "Inspect every full-resolution Poppler and MuPDF page, bind per-page findings, create the byte-identical artifact from build-j, then run generate_unit_032_evidence.py.",
    }
