#!/usr/bin/env python3
"""Two-build PDF/render preflight scaffold for O013 Li Unit 033.
...
``check_unit_033_structure.py`` and the observed nonfatal underfull-box count
...
PDF_NAME = "unit-033-bab-4-grup-simetris.pdf"
DEFAULT_BUILDS = {
    "build-i": ROOT / "build/unit-033-final-m" / PDF_NAME,
    "build-j": ROOT / "build/unit-033-final-n" / PDF_NAME,
}
EXPECTED_METADATA = {
    "/Creator": "LaTeX with hyperref",
    "/Title": "Metode Aljabar, Jilid 1: Arsitektur Dasar - Unit 33: Grup Simetris",
    "/Subject": "Terjemahan Bahasa Indonesia independen; Bagian 4.9 lengkap",
    "/Author": "Wen-Wei Li",
    "/Keywords": "aljabar, teori grup, grup simetris, siklus, grup selang-seling, grup kepang, grup Coxeter, id-ID",
...
def require_admitted_candidate() -> dict[str, Any]:
    ...
    checks = []
    for script in ("check_unit_033_candidate.py", "check_unit_033_structure.py"):
        run = subprocess.run(
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "build/unit-033-preflight")
    parser.add_argument(
    ...
    assert out.parent == (ROOT / "build").resolve() and out.name.startswith("unit-033-preflight")
    ...
    contact_sheets = []
    with tempfile.TemporaryDirectory(prefix="unit033-preflight-") as temporary:
        work = Path(temporary)
    ...
    packet = {
        ...
        "next": "Inspect every full-resolution Poppler and MuPDF page, bind one finding per page, create the byte-identical artifact from build-j, then run generate_unit_033_evidence.py.",
    }
