[CmdletBinding()]
param(
    [string]$OutputDirectory = "build/unit-030-replay"
)
...
$expectedTargetLineRecords = 1895
$expectedTargetBytes = 172726
$expectedTargetSha256 = "245a891930cefb1c18cbd1208386ba5131c56b8b5930510c329577eeeb96cddc"
$spanStart = 794
$spanEnd = 932
$expectedSpanLineRecords = 139
$expectedSpanBytes = 10044
$expectedSpanSha256 = "7e39460c871f38145772d66c95160214d3bf33f18c15f858b4ee874e65474b4b"
...
if ($targetText.Contains("`r")) {
    throw "Unit 030 canonical target must use LF-only line endings"
}
...
if ($targetLineRecords -ne $expectedTargetLineRecords -or $targetItem.Length -ne $expectedTargetBytes -or $targetHash -ne $expectedTargetSha256) {
    throw "Unit 030 canonical target identity mismatch: records=$targetLineRecords bytes=$($targetItem.Length) sha256=$targetHash"
}
...
if ($spanLines.Count -ne $expectedSpanLineRecords -or $spanBytes.Length -ne $expectedSpanBytes -or $spanHash -ne $expectedSpanSha256) {
    throw "Unit 030 canonical span identity mismatch: records=$($spanLines.Count) bytes=$($spanBytes.Length) sha256=$spanHash"
}
...
$jobName = "unit-030-bab-4-deret-komposisi-grup"
