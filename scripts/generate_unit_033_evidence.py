#!/usr/bin/env python3
"""Finalize fail-closed PDF and all-page visual evidence for Unit 033.

Run only after the Unit 033 preflight passes, every full-resolution page has
...
import preflight_unit_033 as preflight


ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PREFLIGHT = ROOT / "build/unit-033-preflight-final-m-n/preflight-observation.json"
DEFAULT_FINDINGS = ROOT / "build/unit-033-visual-findings.json"
DEFAULT_ARTIFACT = ROOT / "artifacts/unit-033-bab-4-grup-simetris-id.pdf"
DEFAULT_EVIDENCE = ROOT / "qa/unit-033-evidence"
DEFAULT_REPORT = ROOT / "qa/UNIT_033_VISUAL_QA_20260826.md"
DEFAULT_PREFLIGHT_REPORT = ROOT / "qa/UNIT_033_VISUAL_PREFLIGHT_20260826.md"
DEFAULT_FINAL_LOG = ROOT / "qa/UNIT_033_BUILD_FINAL.log"
...
def bind_observation(path: Path) -> tuple[dict[str, Any], dict[str, Path]]:
    ...
    assert packet["schema_version"] == 1 and packet["unit"] == "O013-LI-U033"
...
def write_reports(
    ...
    report.write_text(
        f"""# Unit 033 visual and PDF QA - 2026-08-26
...
""",
    ...
    preflight_report.write_text(
        f"""# Unit 033 visual preflight - 2026-08-26
...
def main() -> None:
    ...
    contact_sheets = []
    with tempfile.TemporaryDirectory(prefix="unit033-evidence-") as temporary:
        work = Path(temporary)
        for engine in renderings:
