#!/usr/bin/env python3
"""Build the reader-first checkpoint through complete Chapter 5."""
...
BASE_SCRIPT = ROOT / "scripts/build_checkpoint_reader_0_6_0.py"
OUTPUT = ROOT / "output/pdf/00-metode-aljabar-jilid-1-id-checkpoint-through-bab-5-reader.pdf"
COVER = ROOT / "tmp/pdfs/checkpoint-through-bab-5-cover.pdf"
RECEIPT = ROOT / "qa/unit-043-evidence/checkpoint-through-bab-5-build.json"
PAGE_SIZE = (498.9, 708.66)
FONT_DIR = Path("C:/Windows/Fonts")
REGULAR_FONT = "CheckpointChapter5NotoSans"
BOLD_FONT = "CheckpointChapter5NotoSans-Bold"
ITALIC_FONT = "CheckpointChapter5NotoSans-Italic"


def load_base_units() -> list[tuple[str, str, int, str]]:
    spec = importlib.util.spec_from_file_location("checkpoint_0_6_0", BASE_SCRIPT)
...
UNITS = load_base_units() + [
    ("Bab 4 - Semigrup, Monoid, dan Grup", "unit-025-bab-4-semigrup-monoid-dan-grup-id.pdf", 10, "511d1c0889c0882639be49d00580c0634de7e3074c757616ac10a3f2fa854615"),
    ("Bab 4 - Homomorfisme dan Grup Hasil Bagi", "unit-026-bab-4-homomorfisme-dan-grup-hasil-bagi-id.pdf", 9, "e3c0e0241901eb0f5f2477a1fe09f64eff34af325dc209b25aa8d71900deb089"),
    ("Bab 4 - Produk Langsung, Semilangsung, dan Ekstensi", "unit-027-bab-4-produk-langsung-semilangsung-dan-ekstensi-grup-id.pdf", 7, "8eeab2d34a745b0e5a12acc29c0c5474e9c84d1248686d743302c03859851dd7"),
    ("Bab 4 - Aksi Grup dan Prinsip Pencacahan", "unit-028-bab-4-aksi-grup-dan-prinsip-pencacahan-id.pdf", 7, "50c40ddefa870866568f8d1621d5fc204a1fd0fd0a45bdfc74659197c585790a"),
    ("Bab 4 - Teorema Sylow", "unit-029-bab-4-teorema-sylow-id.pdf", 6, "64fde52948525fc5b159f76fbed6571c150e4184a87e47e715164aec135e5012"),
    ("Bab 4 - Deret Komposisi Grup", "unit-030-bab-4-deret-komposisi-grup-id.pdf", 7, "43ad2ffa2516f2f4394bcb82ad2e585f21c1e9e36a87870f4406a78597f18d74"),
    ("Bab 4 - Grup Solvabel dan Nilpoten", "unit-031-bab-4-grup-solvabel-dan-nilpoten-id.pdf", 9, "313667c3f87439ccaac3f8708653bb352af0ba7a16c9d09b159ad1b836cc32fb"),
    ("Bab 4 - Grup Bebas", "unit-032-bab-4-grup-bebas-id.pdf", 13, "904330916e20f0782b6464cb85e07001851940f4adf153f6592cd34087dbadbf"),
    ("Bab 4 - Grup Simetris", "unit-033-bab-4-grup-simetris-id.pdf", 10, "0af07d45c9aee57e28a6f27fe6162afda253e15c44779ccf07ac591516bd1f1d"),
    ("Bab 4 - Limit dan Pelengkapan Grup", "unit-034-bab-4-limit-dan-kompletisasi-grup-id.pdf", 9, "e69eef970ade092dae4d0e8740092ae8611010bca83ab190e3331e145e852272"),
    ("Bab 4 - Grup dalam Kategori dan Latihan", "unit-035-bab-4-grup-dalam-kategori-dan-latihan-id.pdf", 9, "1cf97dd523ae1a8c5185c4b22a8e6b0dab6e7514ab5387c34959c417f4e35442"),
    ("Bab 5 - Pengantar Teori Gelanggang (lengkap)", "unit-043-bab-5-pengantar-teori-gelanggang-id.pdf", 60, "08fc765af88eea5bb1cb2181223c4217cbeee17a5ec28621cb79193491196762"),
]

# Half-open component ranges. The checkpoint cover itself is physical page 1.
GROUPS = [
    ("Pendahuluan", 0, 1),
    ("Bab 1 - Landasan Teori Himpunan", 1, 7),
    ("Bab 2 - Teori Kategori", 7, 18),
    ("Bab 3 - Kategori Monoidal", 18, 24),
    ("Bab 4 - Teori Grup", 24, 35),
    ("Bab 5 - Pengantar Teori Gelanggang", 35, 36),
]
...
def group_start_pages() -> list[int]:
    starts: list[int] = []
    for _, start, _ in GROUPS:
        starts.append(2 + sum(item[2] for item in UNITS[:start]))
    return starts
...
def make_cover(path: Path) -> None:
    ...
    c.drawString(42, height - 38, "CHECKPOINT PEMBACA - MELALUI BAB 5")
    ...
    y = draw_wrapped(
        c,
        "Pendahuluan serta Bab 1 sampai Bab 5 lengkap. Bab 6 sampai Bab 10 belum "
    ...
    y -= 18
    starts = group_start_pages()
    c.setFont(REGULAR_FONT, 9)
    for (title, start, end), page in zip(GROUPS, starts):
        c.setFillColor(blue)
    ...
    c.drawString(42, 17, "36 komponen | Pendahuluan + Bab 1-5 lengkap | pembaca parsial")
    c.showPage()
    c.save()
...
def add_source_outline(writer: PdfWriter, reader: PdfReader, items: list,
    ...
    for item in items:
        ...
        previous = writer.add_outline_item(
            getattr(item, "title", str(item)), page_offset + source_page, parent=parent
        )
...
def build() -> dict[str, object]:
    ...
    writer.add_named_destination("checkpoint-through-chapter-5-cover", 0)
    ...
    writer.add_metadata(
        {
            "/Title": "Metode Aljabar, Jilid 1 - Edisi Bahasa Indonesia - Checkpoint melalui Bab 5",
            "/Author": "Wen-Wei Li",
            "/Subject": "Checkpoint parsial: pendahuluan serta Bab 1 sampai Bab 5 lengkap",
            "/Keywords": "aljabar, teori himpunan, teori kategori, kategori monoidal, teori grup, teori gelanggang, Bahasa Indonesia",
    ...
    receipt = {
        "schema": "o013-checkpoint-through-chapter-5-build-v1",
        "result": "pass",
        "coverage": "Pendahuluan and complete Chapters 1-5",
