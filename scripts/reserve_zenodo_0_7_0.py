#!/usr/bin/env python3
"""Reserve exactly one 0.7.0 draft in the existing Zenodo concept lineage."""
...
def validate_draft(value: object) -> dict[str, object]:
    ...
    need(str(reserved.get("recid", "")) == str(draft_id),
         "Reserved DOI record does not match draft ID")
...
def main() -> int:
    ...
    public = anonymous.get(
        f"https://zenodo.org/api/records/{SOURCE_RECORD}", timeout=(30, 90)
    )
    ...
    need(record.get("metadata", {}).get("version") == "0.6.0",
         "Source record is no longer the expected 0.6.0 version")

    session = requests.Session()
    session.trust_env = False
    session.headers.update({
        "Authorization": f"Bearer {token()}",
        "User-Agent": "Codex O013 Zenodo 0.7.0 reservation client",
    })
    endpoint = (
        f"https://zenodo.org/api/deposit/depositions/{SOURCE_RECORD}/actions/newversion"
    )
    response = session.post(endpoint, timeout=(30, 120))
    if response.status_code in {200, 201, 202}:
        envelope = response.json()
    elif response.status_code == 400:
        # A prior interrupted reservation may already exist. Follow only the
        # exact latest-draft link exposed by the known source record.
        source = session.get(
            f"https://zenodo.org/api/deposit/depositions/{SOURCE_RECORD}",
            timeout=(30, 90),
        )
        need(source.status_code == 200,
             f"Cannot resolve existing new-version draft: HTTP {source.status_code}")
    ...
    if latest:
        ...
        need(fetched.status_code == 200,
             f"Cannot read latest new-version draft: HTTP {fetched.status_code}")
