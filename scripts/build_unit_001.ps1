[CmdletBinding()]
param(
    [string]$OutputDirectory = "build/unit-001-replay"
)
...
[Environment]::SetEnvironmentVariable("SOURCE_DATE_EPOCH", "1784878369", "Process")
[Environment]::SetEnvironmentVariable("FORCE_SOURCE_DATE", "1", "Process")

Push-Location $sourceRoot
try {
    Invoke-Checked "xelatex" $xelatexArguments
    Invoke-Checked "biber" @("--input-directory", $outputRoot, "--output-directory", $outputRoot, "unit-001-pendahuluan")

    Push-Location $outputRoot
    try {
        Invoke-Checked "makeindex" @("unit-001-pendahuluan.idx")
    }
    finally {
        Pop-Location
    }
...
$pdf = Get-Item -LiteralPath (Join-Path $outputRoot "unit-001-pendahuluan.pdf")
