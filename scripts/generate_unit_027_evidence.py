#!/usr/bin/env python3
"""Generate deterministic canonical PDF evidence for O013 Unit 027."""
...
OUT = ROOT / "qa/unit-027-evidence"
REPORT = ROOT / "qa/UNIT_027_VISUAL_QA_20260825.md"
DPI = 144
PAGES = 7
PIXEL_SIZE = (998, 1418)
EDGE_BAND = 3
PDF_NAME = "unit-027-bab-4-produk-langsung-semilangsung-dan-ekstensi-grup.pdf"
DOCS = {
    "build-c": (
        ROOT / "build/unit-027-c-20260825" / PDF_NAME,
        97_436,
        "c8ec8a08e94c9f762b94029bc9b2fe68d4bf1e2242511f2464958c0548b817c3",
    ),
    "build-d": (
        ROOT / "build/unit-027-d-20260825" / PDF_NAME,
        97_427,
        "8eeab2d34a745b0e5a12acc29c0c5474e9c84d1248686d743302c03859851dd7",
    ),
    "artifact": (
        ROOT / "artifacts/unit-027-bab-4-produk-langsung-semilangsung-dan-ekstensi-grup-id.pdf",
        97_427,
        "8eeab2d34a745b0e5a12acc29c0c5474e9c84d1248686d743302c03859851dd7",
    ),
}
FINAL_LOG = (
    ROOT / "qa/UNIT_027_BUILD_FINAL.log",
    86_569,
    "b18386c3273612276813fa1c9fa00a606becc3d35e980e68351f258bfc893cb4",
)
EXPECTED_METADATA = {
    "/Creator": "LaTeX with hyperref",
    "/Title": "Metode Aljabar, Jilid 1: Arsitektur Dasar - Unit 27: Produk Langsung, Produk Semilangsung, dan Ekstensi Grup",
    "/Subject": "Terjemahan Bahasa Indonesia independen; Bagian 4.3 lengkap",
    "/Author": "Wen-Wei Li",
    "/Keywords": "aljabar, teori grup, produk langsung, produk semilangsung, grup dihedral, ekstensi grup, id-ID",
    "/Producer": "MiKTeX-dvipdfmx (20260404)",
    "/CreationDate": "D:20260825000000Z",
}
EXPECTED_OUTLINE = [
    ("4.3 Produk Langsung, Produk Semilangsung, dan Ekstensi Grup", 3),
    ("Indeks Istilah", 7),
    ("Indeks Simbol", 7),
]
VISUAL_FINDINGS = [
    "Cover hierarchy, wrapped Unit 27 subtitle, scope box, date, and trim margins are balanced and readable.",
    ...
    "Section 4.3 heading reflows cleanly; direct-product definition, bullets, displayed projection, universal diagram, lemma, and proof remain centered and readable.",
    "Semidirect-product definition, numbered explanation, equation (4.1), and Lemma 4.3.4 are aligned with no overflow.",
    "Proof, dihedral-group example, all three polygon/reflection diagrams, captions, and Lemma 4.3.6 have clear strokes and labels.",
    "Internal direct-product display, exact-sequence definition, equations, extension diagram, and page transition are centered and unclipped.",
    "Split-extension diagrams, two-column term index, and symbol index are readable; lower-page whitespace is intentional.",
]
...
def inspect_document(name: str, path: Path, work: Path) -> dict[str, Any]:
    ...
    assert action_counts == Counter({"/GoTo": 10, "/URI": 3})
    ...
    assert len(font_records) == 22 and all(item["embedded"] for item in font_records)
    assert len(font_rows) == 22
    assert all(
    ...
    assert nul_counts == {"pypdf": 8, "pdftotext_layout": 0, "mupdf": 1}
...
def write_report(
    ...
    report = f"""# Unit 027 visual and PDF QA — 2026-08-25
...
3. The final log has 3 LaTeX release warnings, 1 xeCJK warning, 1 frozen `braids` warning, 6 fontspec CJK advisories, 4 visually benign underfull hboxes (badness {', '.join(map(str, log['underfull_hbox_badness']))}), and 1 visually benign underfull vbox (badness {', '.join(map(str, log['underfull_vbox_badness']))}). Fatal/error, unresolved-reference/citation, missing-character, and overfull diagnostics are zero.
...
def main() -> None:
    ...
    assert resolved_output.name == "unit-027-evidence"
    if OUT.exists():
        shutil.rmtree(OUT)
    ...
    contact_sheets = []
    with tempfile.TemporaryDirectory(prefix="unit027-evidence-") as temporary:
        work = Path(temporary)
    ...
    render_inventory = {
        ...
        "warnings": [
            ...
            "Three mathematics fonts lack Unicode maps; stable pypdf/MuPDF NUL counts are disclosed while Poppler text and visible rendering pass.",
        ],
    }
    structure = {
        "status": "PASS_WITH_WARNINGS",
