[CmdletBinding()]
param(
    [string]$OutputDirectory = "build/unit-024-replay"
)
...
$expectedTargetBytes = 89608
$expectedTargetSha256 = "443b71b515aef66c6ba8e259e65083604d227370c1ee7ca3ed49bdb5996f45fb"
$spanStart = 872
$spanEnd = 910
$expectedSpanLineRecords = 39
$expectedSpanBytes = 6071
$expectedSpanSha256 = "576c39746534853cd5127298cf0c2ba7f6afb239e4d7b83f368b7a9969c5f43a"
...
if ($targetText.Contains("`r")) {
    throw "Unit 024 canonical target must use LF-only line endings"
}
...
if ($targetLineRecords -ne $expectedTargetLineRecords -or $targetItem.Length -ne $expectedTargetBytes -or $targetHash -ne $expectedTargetSha256) {
    throw "Unit 024 canonical target identity mismatch: records=$targetLineRecords bytes=$($targetItem.Length) sha256=$targetHash"
}
...
if ($spanLines.Count -ne $expectedSpanLineRecords -or $spanBytes.Length -ne $expectedSpanBytes -or $spanHash -ne $expectedSpanSha256) {
    throw "Unit 024 canonical span identity mismatch: records=$($spanLines.Count) bytes=$($spanBytes.Length) sha256=$spanHash"
}
...
$jobName = "unit-024-bab-3-latihan-kategori-monoidal"
$driverName = "unit-024-bab-3-latihan-kategori-monoidal.tex"
$xelatexArguments = @(
    "-no-shell-escape",
