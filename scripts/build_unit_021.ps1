[CmdletBinding()]
param(
    [string]$OutputDirectory = "build/unit-021-replay"
)
...
$spanStart = 306
$spanEnd = 511
$expectedSpanLineRecords = 206
$expectedSpanBytes = 17968
$expectedSpanSha256 = "57f5bc8a211b6a9b76a096742fbfc94989c890f11d5140ad449d0e76e2c67085"
...
if ($targetText.Contains("`r")) {
    throw "Unit 021 canonical target must use LF-only line endings"
}
$targetLineRecords = ([regex]::Matches($targetText, "`n")).Count
if ($targetLineRecords -ne $expectedTargetLineRecords) {
    throw "Unit 021 canonical target line-record mismatch: records=$targetLineRecords"
}
...
if ($spanLines.Count -ne $expectedSpanLineRecords -or $spanBytes.Length -ne $expectedSpanBytes -or $spanHash -ne $expectedSpanSha256) {
    throw "Unit 021 canonical span identity mismatch: records=$($spanLines.Count) bytes=$($spanBytes.Length) sha256=$spanHash"
}
...
$jobName = "unit-021-bab-3-struktur-kepang"
$driverName = "unit-021-bab-3-struktur-kepang.tex"
$xelatexArguments = @(
    "-no-shell-escape",
