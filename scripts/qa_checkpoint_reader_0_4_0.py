#!/usr/bin/env python3
"""Bounded structural and rendered-page QA for checkpoint reader 0.4.0."""
...
PDF = ROOT / "output" / "pdf" / "00-metode-aljabar-jilid-1-id-checkpoint-0.4.0-reader.pdf"
RENDER_DIR = ROOT / "tmp" / "pdfs" / "checkpoint-0.4.0-mupdf-final"
POPPLER_RENDER_DIR = ROOT / "tmp" / "pdfs" / "checkpoint-0.4.0-poppler-final"
REFERENCE_DIR = ROOT / "tmp" / "pdfs" / "checkpoint-0.4.0-unit-reference"
EVIDENCE_DIR = ROOT / "qa" / "checkpoint-0.4.0-evidence"
...
UNITS = [
    ...
    ("unit-010-bab-2-fungtor-dan-transformasi-natural.pdf", 15),
    ("unit-011-bab-2-kategori-fungtor.pdf", 7),
    ("unit-012-bab-2-sifat-universal-dan-kategori-koma.pdf", 10),
]
EXPECTED_STARTS = [2, 23, 35, 46, 54, 66, 75, 79, 84, 97, 112, 119]
EXPECTED_MODEL = "OpenAI Codex gpt-5.6-sol, Ultra."
EXPECTED_COVER_PHRASES = [
    "Pendahuluan lengkap",
    "Bab 1 lengkap",
    "pengantar Bab 2",
    "Bagian 2.1-2.4 lengkap",
    "Bagian 2.5 dan seluruh bagian sesudahnya belum disertakan",
    "Wen-Wei Li",
    "CC BY 4.0",
    "CC BY-SA 3.0",
    "OFL 1.1",
...
def main() -> None:
    ...
    if len(reader.pages) != 128:
        raise RuntimeError(f"page count is {len(reader.pages)}, expected 128")
    ...
    if page_sizes != Counter({(498.9, 708.66): 128}):
        raise RuntimeError(f"unexpected page geometry: {page_sizes}")
    if rotations != Counter({0: 128}):
    ...
    if len(rendered_pages) != 128:
        raise RuntimeError(f"MuPDF rendered {len(rendered_pages)} pages, expected 128")
    poppler_rendered_pages = sorted(POPPLER_RENDER_DIR.glob("page-*.png"))
    if len(poppler_rendered_pages) != 128:
        raise RuntimeError(
            f"Poppler rendered {len(poppler_rendered_pages)} pages, expected 128"
        )
    reference_pages = render_unit_references()
    if len(reference_pages) != 127:
        raise RuntimeError(f"unit references rendered {len(reference_pages)} pages, expected 127")
    ...
    result = {
        ...
        "coverage": "Pendahuluan lengkap; Bab 1 lengkap; pengantar Bab 2; Bagian 2.1-2.4 lengkap; Bagian 2.5 dan sesudahnya belum disertakan.",
        ...
        "render_qa": {
            ...
            "source_to_checkpoint_pixel_identical_pages": 127,
            "raster_mismatches": 0,
            "blank_pages": blank_pages,
            "page_records": rendered_records,
            "contact_sheets": contact_sheets,
            "visual_review": (
                "PASS: all eleven contact sheets covering pages 1-128 and all twelve "
