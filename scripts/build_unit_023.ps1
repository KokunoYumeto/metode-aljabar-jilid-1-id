[CmdletBinding()]
param(
    [string]$OutputDirectory = "build/unit-023-replay"
)
...
$spanStart = 722
$spanEnd = 871
$expectedSpanLineRecords = 150
$expectedSpanBytes = 14894
$expectedSpanSha256 = "c15e079bc551b30ad7cc6daf72bee58a90108dc7fa5f101f768275e99d1dad05"
...
if ($targetText.Contains("`r")) {
    throw "Unit 023 canonical target must use LF-only line endings"
}
$targetLineRecords = ([regex]::Matches($targetText, "`n")).Count
if ($targetLineRecords -ne $expectedTargetLineRecords) {
    throw "Unit 023 canonical target line-record mismatch: records=$targetLineRecords"
}
...
if ($spanLines.Count -ne $expectedSpanLineRecords -or $spanBytes.Length -ne $expectedSpanBytes -or $spanHash -ne $expectedSpanSha256) {
    throw "Unit 023 canonical span identity mismatch: records=$($spanLines.Count) bytes=$($spanBytes.Length) sha256=$spanHash"
}
...
$jobName = "unit-023-bab-3-sekilas-tentang-2-kategori"
$driverName = "unit-023-bab-3-sekilas-tentang-2-kategori.tex"
$xelatexArguments = @(
    "-no-shell-escape",
