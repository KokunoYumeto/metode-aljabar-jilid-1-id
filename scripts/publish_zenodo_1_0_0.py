#!/usr/bin/env python3
"""Publish and verify complete Li Volume 1 as Zenodo version 1.0.0."""
...
def description(manifest: dict[str, Any]) -> str:
    ...
    return (
        "<p>Edisi Bahasa Indonesia independen dan lengkap dari <em>Methods in Algebra, "
        "Volume 1</em> karya Wen-Wei Li. Versi 1.0.0 memuat pendahuluan dan Bab 1 "
        "sampai Bab 10 lengkap dalam satu pembaca 521 halaman.</p>"
        "<p>Paket pembaca dipimpin oleh PDF; arsip ringkas menyertakan sumber "
        "yang dapat dibangun ulang, backend ID stabil, proyeksi CSV, skrip build, "
        "hak komponen, dan bukti deterministik yang relevan.</p>"
        f"<p>Provenance produksi: {html.escape(release.MODEL)}. Keterangan ini terpisah "
        "dari kepengarangan Wen-Wei Li dan tidak menggantikan kredit penulis, sumber, "
        "atau kontributor manusia.</p>"
        ...
        f"<code>{html.escape(authority['official_pdf_sha256'])}</code>. Arsip versi ini "
        "berasal dari commit edisi "
        ...
        "fragmen AJbook.cls masing-masing CC BY-SA 3.0; font Noto OFL 1.1; font "
        "Fandol 0.3 GPLv3 dengan pengecualian font. 20-LICENSES.md mengendalikan "
        "hak per komponen.</p>"
...
def metadata_for(manifest: dict[str, Any], version_doi: str, draft_id: int) -> dict[str, Any]:
    ...
    result["notes"] = (
        "Status: lengkap untuk Methods in Algebra, Volume 1. Pendahuluan serta Bab 1-10 "
        "seluruhnya disertakan dalam pembaca 521 halaman. Komponen kursus O013 yang "
        "terpisah (Duncan, CRing, dan materi konektif/penguasaan) tidak diklaim sebagai "
        "bagian dari edisi buku ini. Paket multi-lisensi; 20-LICENSES.md dan "
        "RIGHTS_COMPONENTS.csv mengendalikan hak setiap komponen."
    )
    for keyword in ("teori modul", "teori medan", "teori Galois", "valuasi"):
...
def normalize_receipt(path: Path | None) -> None:
    ...
    text = text.replace(
        ...
        "The unchanged work title and Wen-Wei Li creator credit were preserved. Metadata records complete Li Volume 1 coverage, the independent/non-endorsed relationship, exact model provenance, and the component-rights boundary.",
    )
