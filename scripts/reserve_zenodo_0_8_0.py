#!/usr/bin/env python3
"""Reserve exactly one 0.8.0 draft in the existing Zenodo concept lineage."""
...
def main() -> int:
    ...
    need(record.get("metadata", {}).get("version") == "0.7.0", "Source record is no longer version 0.7.0")

    session = requests.Session()
    session.trust_env = False
    session.headers.update({
        "Authorization": f"Bearer {token()}",
        "User-Agent": "Codex O013 Zenodo 0.8.0 reservation client",
    })
