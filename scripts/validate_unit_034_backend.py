#!/usr/bin/env python3
"""Regenerate and fail-closed validate the canonical Unit 034 backend."""
...
import generate_unit_034_backend as gen
import validate_backend as shared_contract


ROOT = Path(__file__).resolve().parents[1]
GENERATOR = ROOT / "scripts/generate_unit_034_backend.py"
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("Unit 034 backend validation refused: " + message)
...
def production_validate() -> dict[str, object]:
    ...
    markers = (
        '"concepts": 419', '"diagrams": 1', '"protected_math_zones": 276',
        '"citation_occurrences": 6', '"digital_reflows": 1',
        '"terminology_normalizations": 0', '"reader_external_reference_isolations": 10',
        '"qa_events": 22',
    )
    ...
    for stale in (b"unit-026", b"u026", b"unit-029", b"u029", b"unit-030", b"u030", b"unit-031", b"u031", b"unit-032", b"u032", b"unit-033", b"u033"):
    ...
    require(data["unit"]["stable_key"] == gen.UNIT_KEY and data["unit"]["order"] == 34,
            "Unit 034 identity/order drift")
    ...
    require(data["workflow"] == {
        "responsible_task": str(uuid.uuid5(namespace, gen.TASK_KEY)), "updated": "2026-08-27",
    ...
    prefix_counts = {
        "concept/": 22, "surface/unit-034/environment/": 25, "surface/unit-034/label/": 11,
        "surface/unit-034/reference/ordinary/": 16, "surface/unit-034/reference/equation/": 0,
        "surface/unit-034/item/": 5, "surface/unit-034/diagram-arrow/": 12,
        "surface/unit-034/drawing-command/": 0, "surface/unit-034/protected-math-zone/": 276,
        "surface/unit-034/terminology-row/": 37, "correction/o013-li-u034-cor-": 1,
        "protected-text-localization/o013-li-u034-loc-": 9,
        "terminology-normalization/o013-li-u034-termnorm-": 0,
        "citation-locator-localization/o013-li-u034-citeloc-": 3,
        "digital-reflow/o013-li-u034-reflow-": 1, "provenance/o013-li-u034-production": 1,
    }
    require(len(concepts) == 419 and len(keys) == len(set(keys)), f"concept census/uniqueness drift: {len(concepts)}")
    ...
    provenance = " ".join(item["text"] for item in concept_by_key["provenance/o013-li-u034-production"]["labels"])
    ...
    exact_record_fragments = {
        "correction/o013-li-u034-cor-001": ("authority line 1720", "baris target 1715", "compatible-family index i >= 0"),
        "digital-reflow/o013-li-u034-reflow-001": ("authority line 1661", "baris target 1656", "display tiga baris"),
    }
    ...
    require([item["bib_key"] for item in data["citations"]] == ["FL14", "Xiong"],
            "two-key citation inventory/order drift")
    require([(item["source_line"], item["target_line"]) for item in data["citations"]]
            == [(1625, 1620), (1658, 1653)], "citation representative-line drift")
    require(len(data["diagrams"]) == 1 and len(data["index_entries"]) == 6, "diagram/index census drift")
    expected_diagram_keys = ["diagram/unit-034/tikzcd-01"]
    require([item["stable_key"] for item in data["diagrams"]] == expected_diagram_keys,
            "single-diagram format/order drift")
    ...
    expected_index_pairs = [
        ("luguoxuji", "poset terarah ke atas (filtered poset)"),
        ("tuopuqun@拓扑群 (topological group)", "grup topologis (topological group)"),
        ("pro-youxianqun@pro-有限群 (pro-finite group)", "grup profinit (pro-finite group)"),
        ("wanbeihua@完备化 (completion)", "pelengkapan (completion)"),
        ("Z_p@$\\Z_p$", "Z_p@$\\Z_p$"),
        ("Tate 模", "modul Tate (Tate module)"),
    ]
    require([(item["source_key"], item["target_key"]) for item in data["index_entries"]]
            == expected_index_pairs, "six-entry localized index order/value drift")
    expected_index_keys = [
        "index-entry/unit-034/main/001", "index-entry/unit-034/main/002",
        "index-entry/unit-034/main/003", "index-entry/unit-034/main/004",
        "index-entry/unit-034/sym1/005", "index-entry/unit-034/main/006",
    ]
    require([item["stable_key"] for item in data["index_entries"]] == expected_index_keys,
            "six-entry index stream/stable-key order drift")
    require(data["unit"]["surface_counts"] == {"sections": 1, "exercises": 0, "hints": 0, "answers": 0, "solutions": 0, "citations": 2, "diagrams": 1, "index_entries": 6}, "native surface census drift")
    ...
    expected_rights = {"rights/principal-cc-by-4.0": True, "rights/lanzhou-cc-by-sa-3.0": False, "rights/ajbook-fragment-cc-by-sa-3.0": True, "rights/noto-fonts-ofl-1.1": True, "rights/fandol-gpl-3.0-with-font-exception": True}
    ...
    fandol_paths = {item["path"] for item in rights["rights/fandol-gpl-3.0-with-font-exception"]["bindings"]}
    require(fandol_paths == {gen.FANDOL_AUTHORITY, gen.FANDOL_LICENSE, gen.FANDOL_README, "repo/source/font-setup-id.tex", gen.ARTIFACT, gen.STRUCTURE_PDF_QA}, "Fandol-rights bindings drift")
    require("font exception" in rights["rights/fandol-gpl-3.0-with-font-exception"]["license"], "Fandol font-exception license drift")
    ...
    expected_qa_keys = [
        "qa/unit-034/admission-gate", "qa/unit-034/source-review",
        "qa/unit-034/candidate-artifact", "qa/unit-034/candidate-check",
        "qa/unit-034/canonical-integration", "qa/unit-034/source-correction",
        "qa/unit-034/digital-reflow", "qa/unit-034/protected-text-localizations",
        "qa/unit-034/citation-locator-localizations", "qa/unit-034/index-localizations",
        "qa/unit-034/terminology-control", "qa/unit-034/terminology-delta",
        "qa/unit-034/terminology-evidence", "qa/unit-034/prepromotion-evidence",
        "qa/unit-034/build-log", "qa/unit-034/driver-external-reference-isolation",
        "qa/unit-034/preflight-tool", "qa/unit-034/evidence-tool",
        "qa/unit-034/visual-preflight", "qa/unit-034/structure-and-pdf-qa",
        "qa/unit-034/render-hash-inventory", "qa/unit-034/all-page-visual-review",
    ]
    ...
    require(uuid_count == 482, f"UUIDv5 entity census drift: {uuid_count}")
    require(
        binding_audit["occurrences"] == 82
        and binding_audit["line_span_occurrences"] == 18
        and binding_audit["unique_paths"] == 44,
    ...
    required_paths = (
        ...
        gen.ARTIFACT, gen.FANDOL_AUTHORITY, gen.FANDOL_LICENSE,
        gen.FANDOL_README,
    )
    ...
    report = {
        "status": "PASS", "unit": "unit-034-bab-4-limit-dan-kompletisasi-grup",
        "authority": "chapter4.tex:1609-1744 (blank line 1744 omitted from mapping)",
        "target": "chapter4.tex:1604-1738", "data": DATA.relative_to(ROOT).as_posix(),
        ...
        "counts": {**gen.EXPECTED_COUNTS, "concepts": 419, "native_citations": 2, "qa_events": 22, "reader_external_reference_isolations": 10, "uuidv5_entities_audited": uuid_count},
        "checks": {
            "deterministic_regeneration_one": "PASS", "deterministic_regeneration_two": "PASS",
            "shared_schema_uuidv5_and_exact_csv": "PASS", "json_csv_roundtrip": "PASS", "exact_419_entity_topology": "PASS",
            "authority_candidate_target_boundary": "PASS", "one_declared_source_correction": "PASS",
            "one_target_only_digital_reflow": "PASS", "nine_protected_text_localizations": "PASS",
            "three_citation_locator_localizations": "PASS", "six_index_localizations": "PASS",
            "ten_external_reference_isolations": "PASS", "component_rights_and_non_endorsement": "PASS",
        ...
        "tools": {
            ...
            SCHEMA.relative_to(ROOT).as_posix(): {"bytes": SCHEMA.stat().st_size, "sha256": sha256(SCHEMA)},
            SHARED_VALIDATOR.relative_to(ROOT).as_posix(): {"bytes": SHARED_VALIDATOR.stat().st_size, "sha256": sha256(SHARED_VALIDATOR)},
        },
    }
