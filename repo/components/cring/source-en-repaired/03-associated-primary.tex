...
\begin{quote}\small\textbf{Edition bridge (original).}
For $x\in M$, write
$\ann_R(x)=\{r\in R:rx=0\}$ for the annihilator of $x$; when the ring is
clear, we abbreviate this to $\ann(x)$.\end{quote}
...
\begin{quote}\small\textbf{Edition bridge (original).}
A nonempty closed subset is irreducible if it is not the union of two proper
closed subsets.  The closure of a point of $\spec R$ is irreducible.\end{quote}
...
\begin{quote}\small\textbf{Edition bridge (original).}
Let $R=k[x,y]$ and $M=R/(x^2,xy)$.  The decomposition
\[
  (x^2,xy)=(x)\cap(x^2,y)
\]
shows that $(x)$ is an isolated associated prime of $M$, whereas the maximal
ideal $(x,y)$ is embedded.\end{quote}
