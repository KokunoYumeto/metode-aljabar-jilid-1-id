...
\begin{example}\label{eg:braid}\index[sym1]{Braid@$\cate{Braid}$}
	...
	\end{tikzpicture}\]
	Ini berarti bahwa $n$ untai \begin{tikzpicture} \draw[lightgray, line width=3pt] (0,0)--(0.5,0); \end{tikzpicture} dipandang sebagai satu kesatuan dan dilintaskan di atas $m$ untai yang dinyatakan oleh \begin{tikzpicture} \draw[black, line width=3pt] (0,0)--(0.5,0); \end{tikzpicture}. Jika $X$ atau $Y$ selanjutnya dibagi menjadi dua kelompok $U, V$ dan diagram kepang yang bersesuaian digambar (berbentuk
...
\begin{Exercises}
	\item Untuk objek-objek $X_1, \ldots, X_{n+1}$ dalam suatu kategori monoidal umum, penerapan $n$ kali operasi $\otimes$ secara berurutan dapat dilakukan dengan berbagai cara, bergantung pada penempatan tanda kurung, seperti $\cdots \otimes (X_i \otimes X_{i+1}) \cdots$ dan seterusnya. Buktikan bahwa terdapat tepat $\frac{1}{n+1} \binom{2n}{n}$ cara untuk menempatkan tanda kurung. \begin{hint} Ini adalah bilangan Catalan dalam kombinatorika. \end{hint}
	\item Buktikan bahwa, untuk setiap kategori monoidal, $\End(\munit)$ komutatif terhadap komposisi morfisme. \begin{hint} Gunakan Lema \ref{prop:Kelly} dan $(f \otimes \identity)(\identity \otimes g) = f \otimes g = (\identity \otimes g) (f \otimes \identity)$.\end{hint}
	\item Tinjau kategori $\cate{On}_f$ yang terdiri atas semua himpunan terurut total berhingga beserta peta-peta pelestari urutan. Untuk himpunan terurut total berhingga $\sigma$, $\tau$, definisikan pada $\sigma \sqcup \tau$ satu-satunya urutan total sedemikian sehingga penyisipan $\sigma$ dan $\tau$ ke dalam $\sigma \sqcup \tau$ melestarikan urutan, dan setiap elemen $\sigma$ lebih kecil daripada setiap elemen $\tau$. Buktikan bahwa $\sqcup$ memberikan struktur kategori monoidal alami pada $\cate{On}_f$, dengan himpunan kosong sebagai objek satuannya.
	\item Melanjutkan soal sebelumnya, tunjukkan bahwa terdapat isomorfisme unik $c(\sigma, \tau): \sigma \sqcup \tau \rightiso \tau \sqcup \sigma$. Apakah keluarga isomorfisme ini membuat $(\cate{On}_f, \sqcup, c)$ menjadi kategori monoidal simetris? Jika tidak, berikan contoh tandingan terhadap naturalitasnya.
	\item Soal ini mengandaikan bahwa pembaca mengenal hasil kali tensor ruang vektor. Misalkan $V$ ruang vektor atas medan $\Bbbk$, dan $R$ automorfisme linear pada $V \otimes V$. Persamaan Yang--Baxter yang bersesuaian didefinisikan sebagai
		\[ (R \otimes \identity_V) \, (\identity_V \otimes R) \, (R \otimes \identity_V) = (\identity_V \otimes R) \, (R \otimes \identity_V) \, (\identity_V \otimes R) , \]
		dengan setiap suku merupakan automorfisme linear pada $V \otimes V \otimes V$. Misalkan $\{e_i\}_{i \in I}$ basis $V$, dan kembangkan $R$ menurut $R(e_i \otimes e_j) = \sum_{k,l} R^{k,l}_{i,j} e_k \otimes e_l$. Maka persamaan di atas menjadi
		\[ \forall i,j,k,l,m,n, \quad \sum_{p,q,y} R^{p,q}_{i,j} R^{y,n}_{q,k} R^{l,m}_{p,y} = \sum_{y,q,r} R^{q,r}_{j,k} R^{l,y}_{i,q} R^{m,n}_{y,r}.  \]
		Jelas bahwa persamaan ini tidak mudah diselesaikan. Jika $\mathcal{V}$ merupakan subkategori monoidal dari $(\cate{Vect}(\Bbbk), \otimes, \ldots)$ dan memiliki struktur kepang $c$, maka untuk setiap $V \in \Obj(\mathcal{V})$, pemilihan $R := c(V,V)$ memberikan solusi persamaan Yang--Baxter. Persamaan-persamaan ini berkaitan erat dengan beberapa sistem integrabel kuantum. Hal ini juga menjelaskan latar belakang Proposisi \ref{prop:YBE-cat-strict}. Verifikasilah pernyataan tersebut.
		
		Sekarang masukkan parameter $q \in \Bbbk^\times$. Untuk ruang vektor-$\Bbbk$ berdimensi hingga $V = \bigoplus_{i=1}^h \Bbbk e_i$, buktikan bahwa
		\[ R(e_i \otimes e_j) = \begin{cases}
			e_j \otimes e_i, & i < j, \\
			q e_i \otimes e_j, & i = j, \\
			e_j \otimes e_i + (q - q^{-1}) e_i \otimes e_j, & i > j
		\end{cases} \]
		memberikan solusi persamaan Yang--Baxter dan memenuhi $\left( R - q \cdot \identity_{V \otimes V} \right) \left( R + q^{-1} \cdot \identity_{V \otimes V} \right) = 0$. Ketika $q=1$, solusi ini mereduksi, pada $\cate{Vect}(\Bbbk)$, menjadi struktur kepang baku $v \otimes w \mapsto w \otimes v$. \index{YBE}

	\item Misalkan $(\mathcal{V}, \otimes, \ldots)$ suatu kategori monoidal. Agar sederhana, selanjutnya andaikan bahwa kategori ini merupakan kategori monoidal ketat, sehingga morfisme kendala asosiativitas dan tanda kurung dapat dihilangkan. Definisikan kategori monoidal $Z(\mathcal{V})$ sebagai berikut:
		\begin{itemize}
			\item Objeknya adalah $(X, \Phi)$, dengan $X$ objek $\mathcal{V}$ dan $\Phi: (X \otimes -) \rightiso (- \otimes X)$ suatu isomorfisme natural antarfungtor. Kita juga mensyaratkan agar diagram berikut komutatif untuk setiap objek $Y,Z$.
				\[\begin{tikzcd}[column sep=small]
				...
				\end{tikzcd}\]
			\item Morfisme dari $(X, \Phi)$ ke $(Y, \Psi)$ adalah semua $f \in \Hom_{\mathcal{V}}(X,Y)$ yang memenuhi
			...
			\item Definisikan $(X, \Phi) \otimes (Y, \Psi)$ sebagai $(X \otimes Y,\; (\Phi \otimes \identity)(\identity \otimes \Psi))$. Definisikan objek satuan dari $Z(\mathcal{V})$ sebagai $\left(\munit, (\identity_X)_{X \in \Obj(\mathcal{V})}\right)$.
		\end{itemize}
	...
	\item Verifikasilah bahwa karakterisasi kategori-$\cate{Ab}$ dalam Contoh \ref{eg:Ab-cat} memang sesuai dengan teori umum kategori diperkaya.
	\item Untuk fungtor-fungtor yang diberikan $\mathcal{A} \xrightarrow{S} \mathcal{C} \xleftarrow{T} \mathcal{B}$, konstruksikan menurut Definisi \ref{def:comma-category} kategori $(S/T)$ (ingat bahwa objeknya berbentuk $(A,B,f)$) beserta fungtor proyeksi $P,Q$. Definisikan transformasi natural $\alpha: SP \to TQ$ melalui $SP(A,B,f) = \left[ SA \xrightarrow{f} TB \right] = TQ(A,B,f)$. Buktikan bahwa $\alpha$ memang merupakan transformasi natural, dan bahwa
		\[\begin{tikzcd}
			(S/T) \arrow[r, "P"] \arrow[d, "Q"'] & \mathcal{A} \arrow[d, "S"] \arrow[Rightarrow, ld, "\alpha" description] \\
			\mathcal{B} \arrow[r, "T"'] & \mathcal{C}
		\end{tikzcd}\]
		dalam $2$-kategori $\cate{Cat}$ merupakan suatu $2$-sel.
\end{Exercises}
